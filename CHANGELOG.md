...
- 🔴 **Починен детект веб-сессии.** hh поменял вёрстку главной: логин переехал в JSON-конфиг страницы (`"login": "..."`), а старый шаблон искал его в другом формате. Из-за этого `test-session` показывал «сессия не активна» при ЖИВОЙ сессии, а `apply-vacancies` зря пропускал все вакансии **с тестом**. Теперь ловим оба формата.
- 🔁 **Retry на сетевые обрывы.** hh изредка рвёт соединение (`RemoteDisconnected`) — раньше один обрыв убивал весь дневной прогон (реальный случай: 8 откликов и падение). Теперь API-клиент делает 3 попытки с паузой, а вакансия, на которой сеть всё же легла, просто пропускается — рассылка продолжается.
- ♻️ **Переотклик по отказам другим резюме** — `apply-vacancies --reapply-rejected --resume-id <другое_резюме>`. Берёт вакансии, где ранее пришёл отказ, и заходит заново другим резюме. Зачем: отказ часто по **грейду** (senior-резюме отклоняют на middle-вакансии), а middle-версией проходишь. `--reapply-experience between1And3` ограничивает грейд вакансий. Дубли отсекаются: если этим резюме уже откликались — пропуск.
- 🧪 **`--dry-run` больше не молчит**: печатает `🧪 [dry-run] откликнулся бы на вакансию ...` и считает итог. Раньше показывал `Отправлено: 0`, что путало.
